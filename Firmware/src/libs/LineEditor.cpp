...
LineEditor::LineEditor(OutputStream *tos)
{
    this->os = tos;
    pos = 0;
    state = NEXT_CHAR;
...
void LineEditor::set_history(std::vector<std::string>& hist, size_t max)
{
    this->history = hist;
    this->history_max = max;
}

void LineEditor::get_history(std::vector<std::string>& hist)
{
    hist = this->history;
}
