...
bool MAX7129::configure(ConfigReader& cr)
{
    ...
    if(!cs->connected()) {
        printf("ERROR:config_max7129: spi cs pin %s is invalid\n", cs_pin.c_str());
        return false;
    }
    printf("DEBUG:config_max7129: cs pin: %s\n", cs->to_string().c_str());
...
void MAX7129::write_register(uint8_t reg, uint8_t data)
{
	cs_select(true);
...
// otherwise blanks
void MAX7129::display_int(int32_t num, bool leading_zeros)
{
	int digit = 0;
	if(num == 0) {
		if(leading_zeros) {
