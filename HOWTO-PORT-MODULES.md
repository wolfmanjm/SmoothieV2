...
NOTE that although it is safe to do prints and SPI calls from a SlowTicker (as it is an RTOS Timer) it should not block as this will block any other timer event as well.
FastTicker callbacks are ISRs and may not do prints or any slow or blocking calls.
